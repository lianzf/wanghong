...
<body class="page-header-fixed page-footer-fixed">
			...
			<div class="container-fluid">
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							<li><i class="icon-list"></i><a href="＃">榜单库</a><i class="icon-angle-right"></i></li>
							<li><a href="#">列表</a></li>
						</ul>
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<span>列表排列顺序创建时间<i class="icon-sort-by-order-alt"></i></span>
								...
								<div id="sample_2_wrapper" class="dataTables_wrapper form-inline" role="grid">
									<div class="row-fluid">
										<div class="control-group">
											<a id="add" class="btn green" ><i class="icon-plus"></i> 添加榜单</a>
										</div>
									</div>
									<div class="row-fluid">
	...
	<script>
				...
				"aoColumns" : [{
					...
					"fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
						...
						if(oData.state == 0 || oData.state == null){
							html += " <a class='btn mini red' href='javascript:;' onclick=confirm('"+oData.id+"',"+oData.state+",'您确认要发布该榜单吗?')><i class='icon-ok'></i> 启用</a>";
						}else{
							html += " <a class='btn mini green' href='javascript:;' onclick=confirm('"+oData.id+"',"+oData.state+",'您确认要停用该榜单吗?')><i class='icon-remove'></i> 停用</a>";
						}
						if (oData.ruleDesc == "人工") {
							html += '<a class="btn mini purple" onclick="addActor('+oData.id+')"><i class="icon-plus"></i> 添加主播</a>';
						}else{
							html += '<a class="btn mini blue" onclick=viewActor('+oData.id+',"'+oData.ruleDesc+'");><i class="icon-search"></i> 查看主播</a>';
						}
						return $(nTd).html(html);	
					}
				} ],
		...
		function edit(id){
			App.Modal.load("./add",{'id' : id},{"title":"修改榜单"});
		}
		
		function addActor(id){
			window.location.href = "./addActor?id="+id;
		}
		
		function viewActor(id,regulation){
			App.Modal.load("./viewActor",{'id' : id,'regulation' : regulation},{"title":"主播列表"});
		}
	</script>
</body>
<!-- END BODY -->
