	...
	<div class="modal-body" data-width="720">
		<div class="portlet-body form row-fluid">
			<form id="form_sample_1" action="#" class="form-horizontal dialog" onsubmit="return false;" enctype="multipart/form-data">
				<input type="hidden" id="initID" name="id" value="${rank.id}"/> 
				<div class="control-group">
					<label class="control-label" for="firstName">名称<span class="required">*</span></label>
					<div class="controls">
						<input type="text" name="displayName" class="m-wrap span10" placeholder="名称" value="${rank.displayName}">
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						<div class="fileupload fileupload-new" data-provides="fileupload">
							<div class="fileupload-new thumbnail" style="width: 150px; height: 115px;">
								<img src="${rank.icon != null && rank.icon != '' ? imageHref : '../media/image/default-img.png'}" alt=""/>
							</div>
				...
				<div class="control-group">
					<label class="control-label" for="firstName">规则</label>
					<div class="controls">
						<select name="regulation" class="m-wrap span10">
							<option>--选择规则--</option>
							<c:forEach items="${rankingActorRegList}" var="actorReg">
								<c:if test="${actorReg.id == rank.regulation}">
									<option value="${actorReg.id}" selected="selected">${actorReg.name}</option>
								</c:if>
								<c:if test="${actorReg.id != rank.regulation}">
									<option value="${actorReg.id}">${actorReg.name}</option>
								</c:if>
							</c:forEach>
						</select>
					</div>
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="number" class="m-wrap span10" name="displayOrder" value="${rank.displayOrder}" maxlength="4" />
				...
				<div class="control-group">
					<div class="control-group">
						...
						<div class="controls">
							<textarea rows="3" class="m-wrap span10" name="description">${rank.description}</textarea>
						</div>
					</div>
				</div>
			</form>
...
$("#submit_btn").click(function(){
	var id = $("#initID").val();
	if(id != "" && id != null){
		$("#form_sample_1").attr("action","./editRankinglist");
	}else{
		$("#form_sample_1").attr("action","./addRankinglist");
	}
