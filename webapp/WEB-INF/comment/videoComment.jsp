<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<div class="tab-pane" id="videoComment">
	<div class="portlet box blue" id="portlet1">
		...
		<div class="portlet-body">
			<div id="sample_1_wrapper" class="dataTables_wrapper form-inline"
				role="grid">
				<div class="row-fluid">
					<select id="state1" class="m-wrap xsmall"></select>
					<!-- 是否可见 -->
					<select id="recommend1" class="m-wrap xsmall"></select>
					<!-- 是否推荐评论 -->
					<select id="isEditorComment1" class="m-wrap xsmall"></select>
					<!-- 是否编辑评论 -->
					<div class="input-prepend">
						<span class="add-on"><i class="icon-calendar"></i></span> <input
							class="m-wrap xsmall date-picker" size="16" id="startTime1"
							placeholder="开始时间" type="text" value="" />
					</div>
					<div class="input-prepend">
						<span class="add-on"><i class="icon-calendar"></i></span> <input
							class="m-wrap xsmall date-picker" size="16" id="endTime1"
							placeholder="结束时间" type="text" value="" />
					</div>
					<input class="m-wrap xsmall" size="10" id="content" type="text" placeholder="正文">
				...
				<div class="row-fluid">
					<table
						...
						id="portlet1_Tables" aria-describedby="sample_2_info">
						<thead>
							<tr>
								<th>ID</th>
								<th>用户ID</th>
...
function confirmVideo(id, state, value) {
	$.ajax({
		type : 'POST',
		url : './onlineComment',
		dataType : "text",
...
function initVideoPage() {
	...
	var recommend = document.getElementById("recommend1").value;
	var isEditorComment = document.getElementById("isEditorComment1").value;
	var startTime = document.getElementById("startTime1").value;
	var endTime = document.getElementById("endTime1").value;
	var content = document.getElementById("content").value;
	var oTable = $('#portlet1_Tables').dataTable({
		...
		"aoColumns" : [{
			...
			"fnCreatedCell" : function(nTd, sData,oData, iRow, iCol) {
				...
				if (oData.state == 0 || oData.state == null) {
					html += " <a class='btn mini red' href='javascript:;' onclick=confirmVideo('"+ oData.id+ "',"+ oData.state+ ",'您确认要显示该评论吗？')><i class='icon-ok'></i> 显示</a>";
				} else {
					html += " <a class='btn mini green' href='javascript:;' onclick=confirmVideo('"+ oData.id+ "',"+ oData.state+ ",'您确认要隐藏该评论吗？')><i class='icon-remove'></i> 隐藏</a>";
				}
...
function editVideo(id) {
	App.Modal.load("./add", {'id' : id, 'table' : 'portlet1'}, {"title" : "修改评论"});
}
</script>
</body>
<!-- END BODY -->
